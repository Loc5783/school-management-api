...
export default function AttendanceClassList() {
  ...
  return <AppShell title="Điểm danh" subtitle="Ghi nhận theo lớp, theo dõi lịch sử và báo cáo vắng mặt.">
    <section className="content-card class-list-card">
      <div className="list-toolbar"><div><p className="card-kicker">DANH SÁCH LỚP</p><h2>{loading ? 'Đang tải lớp học...' : `${visibleClasses.length} lớp đang hoạt động`}</h2></div><div className="toolbar-buttons">{['admin', 'principal', 'teacher'].includes(role) && <Link className="button button-secondary" to="/attendance/reports"><Icon name="chart" size={17} />Báo cáo vắng mặt</Link>}{['admin', 'principal'].includes(role) && <Link className="button button-secondary" to="/attendance/leave-requests"><Icon name="attendance" size={17} />Duyệt đơn nghỉ</Link>}<label className="search-box"><Icon name="search" size={18} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Tìm lớp học..." /></label></div></div>
